...
cursor = cnx.cursor()
sql = "DELETE FROM climate.100degree_cak  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.100degree_cle  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.100degree_eri  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.100degree_mfd  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.100degree_tol  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.100degree_yng  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
...
def sqlSite(site):
   ...
   #Set parameters for data request
    params = {"sid":site.upper()+"thr","sdate":"por","edate":"por","elems":[{"name":"maxt","interval":"mly","duration":"mly","reduce":{"reduce":"cnt_ge_100"},"maxmissing":0}]}
    ...
    for d in data['data']:
        ...
        sql_cvg = "INSERT INTO 100degree_"+site+"(Datetime, year_"+site+", month_"+site+", count_"+site+") VALUES ( '" + datetime + "', '" +  year + "', '" +  month + "', '" + count + "')"  
