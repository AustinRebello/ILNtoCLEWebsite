...
cursor = cnx.cursor()
sql = "DELETE FROM climate.0degree_cak  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.0degree_cle  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.0degree_eri  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.0degree_mfd  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.0degree_tol  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
sql = "DELETE FROM climate.0degree_yng  WHERE Datetime != 0"
cursor.execute(sql)
cnx.commit()
...
def sqlSite(site):
   ...
   #Set parameters for data request
    ...
    params = {"sid":(site.upper()+"thr"),"sdate":"por","edate":"por","elems":[{"name":"mint","interval":"mly","duration":"mly","reduce":{"reduce":"cnt_le_0"},"maxmissing":0}]}
    
    data = GridData(params)

    # Number of sub 32 degree day highs
    paramsh = {"sid":(site.upper()+"thr"),"sdate":"por","edate":"por","elems":[{"name":"mint","interval":"mly","duration":"mly","reduce":{"reduce":"cnt_le_0"},"maxmissing":0}]}
    ...
    for kc in range(len(count)):
        sql_ = "INSERT INTO 0degree_"+site+"(Datetime, year_"+site+", month_"+site+", count_"+site+", counthigh_"+site+") VALUES ( '" + datetime[kc] + "', '" +  year[kc] + "', '" +  month[kc] + "', '" + count[kc] + "', '" + counthigh[kc]+ "')"  
        cursor.execute(sql_)
        cnx.commit()
