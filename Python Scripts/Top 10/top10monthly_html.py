...
def htmlSite(site, porStart):
        ...
        top10.append(
         ...
         ## Output File List ###
        output_file = '../../PDFTxtFiles/Top10/Monthly/'+site.upper()+'/monthly.html'
        ## -- ##
        ...
        for i in range(0,len(month)): # Main for loop to go through and create all data for each month
                ...
                ## Get Top 10 warmest ##
                query = ("SELECT Year, Month, Monthly_Temp_Avg FROM climate.monthly_"+site+" where Month = " + str(i+1) + " AND Monthly_Temp_Avg != 'M' ORDER BY Monthly_Temp_Avg+0 DESC, Year DESC;")
                ...
                ## Get Top 10 coldest ##
                query = ("SELECT Year, Month, Monthly_Temp_Avg FROM climate.monthly_"+site+" where Month = " + str(i+1) + " AND Monthly_Temp_Avg != 'M' ORDER BY Monthly_Temp_Avg+0, Year DESC;")
                cursor.execute(query)
                row = cursor.fetchone()
                ...
                ## Get Top 10 wettest ##
                query = ("SELECT Year, Month, Monthly_Precip_Total FROM climate.monthly_"+site+" where Month = " + str(i+1) + " AND Monthly_Precip_Total != 'M' ORDER BY Monthly_Precip_Total+0 DESC, Year DESC;")
                cursor.execute(query)
                row = cursor.fetchone()
                ...
                ## Get Top 10 driest ##
                query = ("SELECT Year, Month, Monthly_Precip_Total FROM climate.monthly_"+site+" where Month = " + str(i+1) + " AND Monthly_Precip_Total != 'M' ORDER BY Monthly_Precip_Total+0, Year DESC;")
                cursor.execute(query)
                row = cursor.fetchone()
...
print("Generating MONTHLY Top 10 files for NWS CLE")
