...
#    1) 11-26-23: Script created from copy of monthly script
#    2) 11-27-23:
### --- ###
...
def htmlSite(site, porStart):
        ...
        ## Output File List ###
        output_file = '../../PDFTxtFiles/Top10/Yearly/'+site.upper()+'/yearly.html'
        ## -- ##
        ...
        Header_Warm = ('<table width="80%" cellpadding="3" align="center">' + '\n' + '<tr bgcolor="#000000">' + '\n' +
                                '<td colspan="2">' + '\n' +
                                ...
                                site.upper()+' Annual Top 10 Lists' + ' ('+str(porStart)+' - ' + current_year + ') ' + '</p></td>' + '\n' +
                                '</tr>' + '\n' +
                                '</table>' + '\n' +
                                '<br />'+ '\n' +
        ...
        ## Get Top 10 warmest ##
        query = ("SELECT Year, Yearly_Temp_Avg FROM climate.yearly_"+site+" where Yearly_Temp_Avg != 'M' ORDER BY Yearly_Temp_Avg+0 DESC, Year DESC;")
        ...
        ## Get Top 10 coldest ##
        query = ("SELECT Year, Yearly_Temp_Avg FROM climate.yearly_"+site+" where Yearly_Temp_Avg != 'M' ORDER BY Yearly_Temp_Avg+0, Year DESC;")
        cursor.execute(query)
        row = cursor.fetchone()
        ...
        ## Get Top 10 wettest ##
        query = ("SELECT Year, Yearly_Precip_Total FROM climate.yearly_"+site+" where Yearly_Precip_Total != 'M' ORDER BY Yearly_Precip_Total+0 DESC, Year DESC;")
        cursor.execute(query)
        row = cursor.fetchone()
        ...
        ## Get Top 10 driest ##
        query = ("SELECT Year, Yearly_Precip_Total FROM climate.yearly_"+site+" where Yearly_Precip_Total != 'M' ORDER BY Yearly_Precip_Total+0, Year DESC;")
        cursor.execute(query)
        row = cursor.fetchone()
        ...
        ## Get Top 10 snowiest ##
        query = ("SELECT Year, Yearly_Snow_Total FROM climate.yearly_"+site+" where Yearly_Snow_Total != 'M' AND Yearly_Snow_Total != 'T' ORDER BY Yearly_Snow_Total+0 DESC, Year DESC;")
        cursor.execute(query)
        row = cursor.fetchone()
        ...
        query = ("SELECT Year, Yearly_Snow_Total FROM climate.yearly_"+site+" where Yearly_Snow_Total != 'M' AND Yearly_Snow_Total != 'T' ORDER BY Yearly_Snow_Total+0 ASC, Year DESC;")
        cursor.execute(query)
        row = cursor.fetchone()
        ...
        print('Now creating pdf files for '+site)

        pdfkit.from_file( '../../PDFTxtFiles/Top10/Yearly/' + site.upper() + '/yearly.html', '../../PDFTxtFiles/Top10/Yearly/' + site.upper() + '/yearly.pdf')

print("Generating YEARLY Top 10 files for NWS CLE")
